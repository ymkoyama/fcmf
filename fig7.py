...
def filename(param, a, b, MDS, prec, phi, name):
    return "inverse_power" + param + "_a" + a + "_b" + b \
        + "_gauss_MDS" + str(MDS) + "_prec" + str(prec) \
        + "/gauss_" + phi + "_" + name + "_M" + f"{M:04}" + ".dat"
param = ["0.5", "1", "2"]
log2a = [-1,  -10]
prec  = [248, 184]
xmin  = [10,  10]
...
M = 17
MDS_list = []
MDS_list.append([24, 48, 96, 192])
MDS_list.append([24, 48, 96, 192, 384, 768, 1536, 3072])
...
for i in range(nrows):
    for j in range(ncols):
        ...
        print("MRE", "param", param[i], "a", a[j], "b", b, "M", M)
        ...
        if j == 0:
            ax.set_ylabel(r"$\log_{10}(\mathrm{MRE})$")
